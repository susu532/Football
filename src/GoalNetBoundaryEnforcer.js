...
/**
 * GoalNetBoundaryEnforcer.js (Client ES Module Version)
 * 
 ...
 * for client-side prediction matching server-side physics.
 */
...
export const GOAL_NET_BOUNDARY = {
 ...
 */
export function detectBallZone(pos, ballRadius) {
 ...
 */
export function smartClamp(value, velocity, minBound, maxBound, restitution = 0.5) {
 ...
 */
export function smartClampGoalNetZ(z, vz, ballRadius, goalRestitution = 1.5) {
 ...
 */
export function enforceGoalNetBoundaries(pos, vel, ballRadius) {
  // Use PHYSICS constants
  const WALL_RESTITUTION = PHYSICS.WALL_RESTITUTION || 0.3
  const GOAL_RESTITUTION = PHYSICS.GOAL_RESTITUTION || 1.5
  const GROUND_RESTITUTION = PHYSICS.GROUND_RESTITUTION || 0.9
  ...
  if (zone.isDeepInGoal) {
    ...
    if (Math.abs(pos.z) > goalSideLimit) {
      if (zone.isWithinGoalWidth) {
        ...
        // Push back to arena boundary instead
 ...
 */
export function sweepGoalNetBoundary(startPos, endPos, ballRadius) {
 ...
 */
export function applyCCDBoundaryCorrection(pos, vel, dt, ballRadius, restitution = 1.5) {
  const endPos = {
    x: pos.x + vel.x * dt,
    y: pos.y + vel.y * dt,
    z: pos.z + vel.z * dt
